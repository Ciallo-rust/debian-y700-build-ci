...
根据上游@GUF296大佬的Ubuntu构建CI改的Debian构建CI。自用，技术力非常渣。boot.img直接使用上游release。snap禁用关闭，firefox安装关闭，以我能力改不了这两个就先关闭免得出问题。由于依赖问题，Debian选用forky\(testing\)而不是trixie。目前CI构建已知问题没有声音。
